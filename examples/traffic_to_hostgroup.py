...
class TrafficToHostGroupApp(NetProfilerApp):
    def add_options(self, parser):
        ...
        group.add_option(
            ...
            help='[REQUIRED] type and group to add gathered data to separated by a :, ex: "ByLocation:SanFrancisco"')
        group.add_option(
    ...
    def main(self):
        ...
        # Fix the group argument's format and break it into parts for readability
        ...
        for i in range(len(data)):
            if data[i][1] == type_and_group:
                new_config_entries.append({'cidr': data[i][0] + '/32', 'name': group_name})
        # Make sure that if there were no entries returned, we don't overwrite the old data
        ...
        host_types = self.netprofiler.api.host_group_type.get_all()
        target_type_id = -1
        ...
        if target_type_id == -1:
            print('ERROR: Host Type: "' + type_name + '" was not found.')
            return

        # Get the current config from the target host group
        config = self.netprofiler.api.host_group_type.get_config(target_type_id)
        old_config_size = len(config)
        # If the append flag is not true, remove all entries in config matching group_name
        ...
        if self.netprofiler.api.host_group_type.set_config(target_type_id, config) is None:
            print("Successfully updated type: " + type_name + ", group: " + group_name)
